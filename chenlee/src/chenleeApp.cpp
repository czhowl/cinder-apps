...
void chenleeApp::setup()
{
    ...
    r = 2;
//    mParams->addParam( "Randomness", &r ).min( 0.1f ).max( 5.0f ).keyIncr( "q" ).keyDecr( "a" ).precision( 2 ).step( 0.01f );
    mFont1 = Font("Arial", 12.0f);
    ...
    mCamera.lookAt( vec3( -100.0f, 100.0f, 0.0f ), vec3( 0.0f, 0.0f, 5.0f ) );
...
void chenleeApp::print_device_info(rs2_device* dev)
{
    rs2_error* e = 0;
...
void chenleeApp::check_error(rs2_error* e)
{
    if (e)
    {
...
float chenleeApp::get_depth_unit_value(const rs2_device* const dev)
{
    rs2_error* e = 0;

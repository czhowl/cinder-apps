...
void main( void )
{
    vec2 F;
//    float xt;
//    if(vTexCoord0.x < 0.5) xt = vTexCoord0.x * 30;
//    else xt = (1.0 - vTexCoord0.x) * 30;
//    float xt = cos( (vTexCoord0.x - 0.5) * PI) * 10;
    float xt = vTexCoord0.x * 10 + noise(Time * 0.1) * 5;
    float yt = vTexCoord0.y * 10 + noise(Time * 0.1 + 100323) * 5;
    float zt = Time * 1.0;
    F = worley3D( vec3(xt + noise(vec2(zt + 10000, yt * 2)) * noise(Time * 0.6) * 0.9, yt + noise(vec2(zt, xt * 2)) *noise(Time * 0.6 + 100323)* 0.9, zt), 1.0, false);
//    F = worley3D( vec3(xt, yt, zt), 1.0, false);
    oColor = vec4(vec3(pow(F.x, 10)) * vec3(0.6,1.0,0.99) * 0.28+ vec3( 0.04, 0.05, 0.16 ), 1.0);
}
